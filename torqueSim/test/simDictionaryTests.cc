...
TEST_CASE("SimNameDictionary inserts finds and removes named objects", "[SimDictionary][Name]") {
   ...
   DictionaryTestObject* first = createRegisteredObjectWithId<DictionaryTestObject>(uniqueName("namedA"), 210001);
   DictionaryTestObject* second = createRegisteredObjectWithId<DictionaryTestObject>(uniqueName("namedB"), 210002);
...
TEST_CASE("SimManagerNameDictionary tracks manager lookups independently", "[SimDictionary][ManagerName]") {
   ...
   DictionaryTestObject* first = createRegisteredObjectWithId<DictionaryTestObject>(uniqueName("managerA"), 220001);
   DictionaryTestObject* second = createRegisteredObjectWithId<DictionaryTestObject>(uniqueName("managerB"), 220002);
...
TEST_CASE("SimIdDictionary inserts finds and removes by id", "[SimDictionary][Id]") {
   ...
   DictionaryTestObject* first = createRegisteredObjectWithId<DictionaryTestObject>(uniqueName("idA"), 230001);
   DictionaryTestObject* second = createRegisteredObjectWithId<DictionaryTestObject>(uniqueName("idB"), 230002);
   DictionaryTestObject* colliding = createRegisteredObjectWithId<DictionaryTestObject>(uniqueName("idCollision"), 230001 + 4096);
...
TEST_CASE("SimNameDictionary and SimManagerNameDictionary resize and rehash cleanly", "[SimDictionary][Name][ManagerName]") {
   ...
   for (U32 i = 0; i < 40; ++i)
   {
      auto* obj = createRegisteredObjectWithId<DictionaryTestObject>(uniqueName("rehash"), 250000 + i);
